...
export interface ScorecardPreviewProps {
  result: LookupResult;
  onResultChange: (r: LookupResult) => void;
...
function EditableCell({
  ...
  return (
    <span
      ...
      className={`cursor-pointer hover:bg-accent/10 rounded px-0.5 ${value === 0 ? "text-red-400 font-bold" : ""} ${className}`}
      title={value === 0 ? "Not found — click to enter value" : "Click to edit"}
    >
      {value === 0 ? "–" : value}
    </span>
  );
  ...
  if (result.source === "photo_ocr") {
    return (
      <p className="text-[0.75rem] text-secondary">
        Source: Uploaded scorecard photo
      </p>
    );
  }
  return null;
      ...
      {!duplicates && (
        <div className="flex gap-3">
          <Button onClick={() => handleSave(false)} disabled={saving}>
            {saving ? "Saving..." : mergeTarget ? "Add Selected Tees" : "Save Course"}
          </Button>
